...
def calc_Kstar(params,star_params):   
    ...
    .. math::       
       P^2/a^3 &= 4 \pi^2 / (G (M_\mathrm{prim}+M_\mathrm{sec}))    \\
       P &= 2 \pi a^{3/2} / \sqrt{ G (M_\mathrm{prim}+M_\mathrm{sec}) } 
    
    Thus the general formula can also write as
    
    .. math:: 
       K_\mathrm{prim} = (G/a)^{1/2} M_\mathrm{sec} \sin(i_p) ((M_\mathrm{sec}+M_\mathrm{prim})^{1/2} / M_\mathrm{prim}) /\sqrt{1-e^2}    
...
def calc_rv_star(coord_vis,system_param,bjd_all,t_dur_all,sysvel,n_ov_all,kepl_pl):
    ...
    Args:
        TBD
        n_ov (float): oversampling factor of exposures
    ...
    #No oversampling
    if n_ov_all is None:
        RV_star_stelCDM=np.zeros(len(bjd_all),dtype=float)

        #Adding each planet contribution to reflex motion
        for pl_loc in kepl_pl:     
            PlParam_loc=system_param[pl_loc]   
            
            #Orbital phase 
            phase=get_timeorbit(coord_vis[pl_loc]['Tcenter'],bjd_all, PlParam_loc, t_dur_all)[1]

            #True anomaly for mid exposure
            #    - dimension n_exp
            True_anom_all=calc_true_anom(PlParam_loc['ecc'],phase,PlParam_loc['omega_rad'])[0]            

            #Keplerian motion induced by the planet (km/s)
            RV_star_stelCDM+=PlParam_loc['Kstar_kms']*(np.cos(True_anom_all+PlParam_loc['omega_rad'])+PlParam_loc['ecc']*np.cos(PlParam_loc['omega_rad']) )                

    #------------------------------------
    
    #Constant oversampling
    elif isinstance(n_ov_all, (int, np.integer)):
        RV_star_stelCDM=np.zeros([n_ov_all,len(bjd_all)],dtype=float)
        for pl_loc in kepl_pl:     
            PlParam_loc=system_param[pl_loc]   
            st_phase,phase,end_phase=get_timeorbit(coord_vis[pl_loc]['Tcenter'],bjd_all, PlParam_loc, t_dur_all)[0:3]
    
            #True anomaly for start and end of exposure
            #    - dimension ( 2 x n_exp)
            True_anom_all=calc_true_anom(PlParam_loc['ecc'],np.vstack((st_phase,end_phase)),PlParam_loc['omega_rad'])[0]

            #Min/max true anomaly for all exposures
            #    - as the min (resp. max) anoamly may correspond to the max (resp. min) phase
            #    - dimension n_exp
            min_TA_all=np.min(True_anom_all,axis=0)
            max_TA_all=np.max(True_anom_all,axis=0)
            
            #Oversampling of true anomaly over exposure
            #    - dimension ( n_ov x n_exp)
            dTrueAnom_all_ov=(max_TA_all-min_TA_all)/n_ov_all            
            True_anom_all_ov=min_TA_all+dTrueAnom_all_ov*(0.5+np.arange(n_ov_all,dtype=float)[:,None])

            #Keplerian motion induced by the planet (km/s)
            #    - dimension ( n_ov x n_exp)
            RV_star_stelCDM+=PlParam_loc['Kstar_kms']*(np.cos(True_anom_all_ov+PlParam_loc['omega_rad'])+PlParam_loc['ecc']*np.cos(PlParam_loc['omega_rad']) )                

        #Average oversampled RV (km/s)
        #    - dimension n_exp
        RV_star_stelCDM=np.mean(RV_star_stelCDM,axis=0)  

    #------------------------------------
    
    #Exposure-specific oversampling
    else:
        cond_ov = (n_ov_all>1)
        cond_no_ov = ~cond_ov
        RV_star_stelCDM=np.zeros(len(bjd_all),dtype=float)
        
        #Exposures without oversampling
        for pl_loc in kepl_pl:     
           PlParam_loc=system_param[pl_loc]   
           phase_no_ov=get_timeorbit(coord_vis[pl_loc]['Tcenter'],bjd_all[cond_no_ov], PlParam_loc, t_dur_all[cond_no_ov])[1]        
           True_anom_no_ov=calc_true_anom(PlParam_loc['ecc'],phase_no_ov,PlParam_loc['omega_rad'])[0]            
           RV_star_stelCDM[cond_no_ov]+=PlParam_loc['Kstar_kms']*(np.cos(True_anom_no_ov+PlParam_loc['omega_rad'])+PlParam_loc['ecc']*np.cos(PlParam_loc['omega_rad']) )              
    
        #Processing oversampled exposures
        for iexp,bjd_exp,t_dur_exp,n_ov_exp in zip(np_where1D(cond_ov),bjd_all[cond_ov],t_dur_all[cond_ov],n_ov_all[cond_ov]):
            RV_star_stelCDM_exp=np.zeros(n_ov_exp,dtype=float)
            for pl_loc in kepl_pl:     
                PlParam_loc=system_param[pl_loc]   
                st_phase_exp,_,end_phase_exp=get_timeorbit(coord_vis[pl_loc]['Tcenter'],bjd_exp, PlParam_loc, t_dur_exp)[0:3]        
                True_anom_ov=calc_true_anom(PlParam_loc['ecc'],np.vstack((st_phase_exp,end_phase_exp)),PlParam_loc['omega_rad'])[0]
                min_TA_ov=np.min(True_anom_ov)
                max_TA_ov=np.max(True_anom_ov)  
                dTrueAnom_ov=(max_TA_ov-min_TA_ov)/n_ov_exp
                True_anom_exp_ov=min_TA_ov+dTrueAnom_ov*(0.5+np.arange(n_ov_exp,dtype=float))
                RV_star_stelCDM_exp+=PlParam_loc['Kstar_kms']*(np.cos(True_anom_exp_ov+PlParam_loc['omega_rad'])+PlParam_loc['ecc']*np.cos(PlParam_loc['omega_rad']) )              
            RV_star_stelCDM[iexp]=np.mean(RV_star_stelCDM_exp) 
...
def Kepler_func(Mean_anom,ecc, tol=1e-10, max_iter=50):
    ...
    To find the eccentric anomaly of the orbit, given the mean anomaly and eccentricity, using vectorized Newton-Raphson.
    
    Args:
        Mean_anom (arr, float): Mean anomaly.
        ecc (float): Eccentricity.
    
    Returns:
        Ecc_anom (arr, float): Eccentric anomaly.
    
    """     
    Ecc_anom = Mean_anom + ecc * np.sin(Mean_anom) * (1. + ecc * np.cos(Mean_anom))
    for _ in range(max_iter):
        f = Ecc_anom - ecc * np.sin(Ecc_anom) - Mean_anom
        f_prime = 1 - ecc * np.cos(Ecc_anom)
        delta = f / f_prime
        Ecc_anom -= delta
...
def calc_mean_anom_TR(ecc,omega_bar):  
    ...
    Returns:
        Mean_anom_TR (float): Mean anomaly at mid-transit. 
...
def calc_true_anom(ecc,phase,omega_bar):
    ...
    #  - omega_bar is the angle between the ascending node and the periastron, in the orbital plane (>0 counterclockwise)                       
        ...
        Ecc_anom = Kepler_func(Mean_anom,ecc) 
...
def calc_pl_coord_plots(n_pts_orbit,pl_params,unit = 'Rs'):  
    ...
    #Elliptic orbit 
        ...
        #Eccentric anomaly
        Ecc_anom_plot=Kepler_func(Mean_anom_plot,ecc)
